...
def handle_text_processing_start(message_content, current_session_id):
    """处理文本处理开始"""
    try:
        # 获取全局管理器
        ...
        if current_state != AppState.IDLE:
            log.warning(f"状态不是IDLE，当前状态: {current_state.value}")
            return False
        ...
        log.info(f"✅ 文本处理开始: {message_content[:50]}...")
        return True
...
def handle_text_processing_complete(message_id, full_content):
    """处理文本处理完成"""
    try:
        # 获取全局管理器
        state_manager = app.state_manager
        ...
        # 取消SSE超时，启动TTS超时
        timeout_manager.cancel_timeout(f"text_sse_{message_id}")
        content_length = len(full_content) if full_content else 0
        timeout_manager.start_timeout(
            timeout_id=f"text_tts_{message_id}",
            content_length=content_length,
...
def handle_text_tts_complete(message_id):
    """处理文本TTS完成"""
    try:
        # 获取全局管理器
        state_manager = app.state_manager
        ...
        # 取消TTS超时
        timeout_manager.cancel_timeout(f"text_tts_{message_id}")
...
def handle_chat_interactions_v2(topic_clicks, send_button_clicks, completion_event_json, transcription_data,
                               message_content, messages_store, current_session_id, ws_connection_data):
    """处理聊天交互 - V2版本，集成新的状态管理器"""
    ...
    log.info(f"🔍 聊天交互V2被触发: {triggered_id}, 当前状态: {current_state.value}")
    ...
    # 处理发送按钮点击
        ...
        # 开始文本处理
        if not handle_text_processing_start(message_content, current_session_id):
            return messages, message_content, False, False, dash.no_update
        ...
        if messages:
            try:
                if completion_event_json:
                    ...
                    # 处理文本处理完成
                    if handle_text_processing_complete(message_id, full_content):
                        # 更新消息内容
...
# 注册回调函数
def register_chat_input_callbacks_v2(app):
    """注册聊天输入回调函数V2"""
    log.info("✅ 聊天输入回调函数V2已注册")
