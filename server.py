...
# 添加SSE流式路由 - 唯一的流式路由实现
@app.server.route('/api/stream', methods=['GET', 'POST'])
@stream_with_context  # 添加这个装饰器确保上下文正确传递
def stream():
    # 支持从URL参数获取数据
    message_id = request.args.get('message_id', str(time.time()))  # 提供默认值
    session_id = request.args.get('session_id')
    messages = request.args.get('messages')
    
    # 也支持从JSON请求体获取数据，增强兼容性
    if request.method == 'POST' and not messages:
    ...
    if messages:
        try:
            ...
            error_message = {
                'id': message_id,
                'content': "Invalid messages format",
                'complete': True
            }
        ...
        error_message = {
            'id': message_id,
            'content': "Missing messages parameter",
            'complete': True
        }
        return Response(f"data: {json.dumps(error_message)}\n\n", content_type='text/event-stream')
        ...
        try:
            ...
            for chunk in yychat_client.chat_completion(
                ...
                if isinstance(chunk, dict):
                    # 检查是否有choices字段并且不为空
                    if 'choices' in chunk and chunk['choices'] and isinstance(chunk['choices'][0], dict):
                        choice = chunk['choices'][0]
                        # 提取content（可能在delta字段中）
